...
export function DetectionResultToExtraction(
...
class ExtractProvider implements CodeActionProvider {
  public async provideCodeActions(
    ...
    const diagnostic = context.diagnostics.find(
      i => i.code === PROBLEM_CODE_HARD_STRING,
    ) as DiagnosticWithDetection | undefined

    // quick fix for hard string problems
    if (diagnostic?.detection) {
      const extract = new CodeAction(
        i18n.t('refactor.extract_text'),
      ...
      const ignoreFileTitle = i18n.t(
        'refactor.extract_ignore_by_file',
        diagnostic.detection.text,
      )
      const ignoreByFile = new CodeAction(
        ignoreFileTitle,
        CodeActionKind.QuickFix,
      )
